...
class Pacientes extends Controllers{
	...
	public function getSelectUsers(){
	    ...
	    if(count($arrData) > 0 ){
	        for ($i=0; $i < count($arrData); $i++) { 
	            $htmlOptions .= '<option value="'.$arrData[$i]['IdUsuario'].'">'.$arrData[$i]['Nombre']."-----".$arrData[$i]['NombrePac'].'</option>';
	        }
	    }
	    echo $htmlOptions;
	    die();        
	...
	public function perfil(){
			...
			$data['page_title'] = "Perfil del paciente";
