...
class Recetas extends Controllers{
	...
	public function getRecetas(int $idmedico)
	{
		$idMedico = intval($idmedico);
		$arrData = $this->model->selectRecetas($idMedico);
	...
	public function getSelectConsultas(int $idmedico){
		$idMedico = intval($idmedico);
		$htmlOptions = "";
		$arrData = $this->model->selectConsultas($idMedico);
		if(count($arrData) > 0 ){
			for ($i=0; $i < count($arrData); $i++) { 
				$htmlOptions .= '<option value="'.$arrData[$i]['IdConsulta'].'">'.$arrData[$i]['IdConsulta']."--".$arrData[$i]['NombreMedico']."--".$arrData[$i]['NombrePaciente']."--".$arrData[$i]['FechaConsulta'].'</option>';
			}
		}
		echo $htmlOptions;
		die();		
