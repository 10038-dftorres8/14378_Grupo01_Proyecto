...
class Misconsultas extends Controllers{
	...
	public function Misconsultas()
	{
		...
		$data['page_tag'] = "Misconsultas";
		$data['page_title'] = "MIS CONSULTAS <small>VERIS</small>";
		$data['page_name'] = "misconsultas";
		$data['page_functions_js'] = "functions_misconsultas.js";
		$this->views->getView($this,"misconsultas",$data);
	}
	...
	public function getConsultas(int $idmedico)
	{
		$idMedico = intval($idmedico);
		$arrData = $this->model->selectConsultas($idMedico);
		for ($i=0; $i < count($arrData); $i++) {
			...
			$btnDelete = '';
				...
				$btnEdit = '<button class="btn btn-primary btn-sm" onClick="fntEditInfo('.$arrData[$i]['IdConsulta'].')" title="Editar consulta" disabled><i class="fas fa-pencil-alt"></i></button>';
