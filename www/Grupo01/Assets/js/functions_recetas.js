...
document.addEventListener('DOMContentLoaded', function(){
    ...
    tableRecetas = $('#tableRecetas').dataTable( {
        ...
        "ajax":{
            "url": " "+base_url+"/Recetas/getRecetas/"+idmedico,
            "dataSrc":""
        },
        "columns":[
            {"data":"IdReceta"},
    ...
    function fntListConsultas(idmedico){
        var idmedico = document.getElementById('userData').getAttribute('data-idmedico');
        if(document.querySelector('#listConsultas')){
            var ajaxUrl = base_url+'/Recetas/getSelectConsultas/'+idmedico;
            var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status){
                ...
                document.querySelector("#celMedico").innerHTML = objData.data.NombreMed;
                document.querySelector("#celPaciente").innerHTML = objData.data.NombrePac;
