var tableMisconsultas;
var divLoading = document.querySelector("#divLoading");
document.addEventListener('DOMContentLoaded', function(){
    var idmedico = document.getElementById('userData').getAttribute('data-idmedico');
    tableMisconsultas = $('#tableMisconsultas').dataTable( {
        "aProcessing":true,
        "aServerSide":true,
        "language": {
        ...
        "ajax":{
            "url": " "+base_url+"/Misconsultas/getConsultas/"+idmedico,
            "dataSrc":""
        },
        "columns":[
            {"data":"IdConsulta"},
    ...
    if(document.querySelector("#formMisconsultas")){
        var formConsulta = document.querySelector("#formMisconsultas");
        formConsulta.onsubmit = function(e) {
            ...
            var ajaxUrl = base_url+'/Misconsultas/setConsulta'; 
            ...
            request.onreadystatechange = function(){
                if(request.readyState == 4 && request.status == 200){
                    ...
                    if(objData.status)
                    {
                        $('#modalFormMisconsultas').modal("hide");
                        formConsulta.reset();
    ...
    function fntListPacientes(){
        if(document.querySelector('#listPacientes')){
            var ajaxUrl = base_url+'/Misconsultas/getSelectPacientes';
            var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    ...
    function fntListMedicos(){
        if(document.querySelector('#listMedicos')){
            var ajaxUrl = base_url+'/Misconsultas/getSelectMedicos';
            var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            request.open("GET",ajaxUrl,true);
            request.send();
            request.onreadystatechange = function(){
...
function fntViewInfo(idconsulta){
    ...
    var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    var ajaxUrl = base_url+'/Misconsultas/getConsulta/'+idconsulta;
    request.open("GET",ajaxUrl,true);
    request.send();
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status){
                ...
                $('#modalViewMiconsulta').modal('show');
            }else{
                swal("Error", objData.msg , "error");
            }
        }
...
function fntEditInfo(idconsulta){
    ...
    request.onreadystatechange = function(){
        ...
        $('#modalFormMisconsultas').modal('show');
    }
}
...
function fntDelInfo(idconsulta){
    ...
    swal({
        ...
        {
            var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            var ajaxUrl = base_url+'/Misconsultas/delConsulta';
...
function openModal()
{
    ...
    document.querySelector("#formMisconsultas").reset();
    $('#modalFormMisconsultas').modal('show');
}
