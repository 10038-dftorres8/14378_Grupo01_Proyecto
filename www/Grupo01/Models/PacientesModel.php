<?php 
	...
	class PacientesModel extends Mysql
	{
		...
		{
			$sql = "SELECT u.*, p.IdPaciente, p.Nombre AS NombrePac FROM usuarios u
		...
		public function updatePerfil(int $idUsuario, string $nombre, int $cedula, int $edad, string $genero, int $estatura, string $peso){
			$this->intIdUsuario = $idUsuario;
			$this->strNombre = $nombre;
			$this->intCedula = $cedula;
			$this->intEdad = $edad;
			...
			$sql = "UPDATE pacientes SET Nombre=?, Cedula=?, Edad=?, Genero=?, Estatura=?, Peso=? 
					WHERE IdPaciente = $this->intIdUsuario ";
			$arrData = array($this->strNombre,
							$this->intCedula,
							$this->intEdad,
							$this->strGenero,
